-- PrimeHub 7.5 release bootstrap
...
local function replaceOnce(source,oldText,newText,label)
    ...
    if not a then error("[PrimeHub 7.5] patch target missing: "..tostring(label or oldText)) end
    return string.sub(source,1,a-1)..newText..string.sub(source,b+1)
end

local function patchRuntime75(src)
    local old,new
...
local __base = ]==========],
    ...
    old=[==========[            "[PrimeHub] PrimeHub 7.5 FRESH REMOTE payload queued for teleport. resume sniper=%s hop=%s esp=%s",
            tostring(env.AutoFruitSniper == true), tostring(env.AutoHop == true), tostring(env.FruitESP == true)
]==========]
    new=[==========[            "[PrimeHub] PrimeHub 7.5 FRESH REMOTE payload queued for teleport. resume sniper=%s hop=%s esp=%s serverOnly=%s",
...
if type(originalLoadstring)~="function" then error("[PrimeHub 7.5] loadstring unavailable") end
local finalPatched=false

local function hookedLoadstring(code,chunkname)
    local runtimeHeader = "-- PrimeHub 7.5 | Fruit Sniper"
    if type(code)=="string" and string.sub(code,1,#runtimeHeader)==runtimeHeader then
        code=patchRuntime75(code)
        finalPatched=true
    end
...
if not okBody or type(src)~="string" or #src<100 then
    ...
    error("[PrimeHub 7.5] Could not download fresh 7.2 base: "..tostring(src))
end

src=string.gsub(src,"7%.2","7.5")
src=string.gsub(src,"PrimeHub_7_2_RELEASE%.lua","PrimeHub_7_5_RELEASE.lua")
src=string.gsub(src,"PrimeHub_7_2_PATCHED%.lua","PrimeHub_7_5_PATCHED.lua")

local fn,err=originalLoadstring(src)
if not fn then
    loadstring=originalLoadstring
    error("[PrimeHub 7.5] Base compile failed: "..tostring(err))
end
local ok,result=pcall(fn)
loadstring=originalLoadstring
if not ok then error("[PrimeHub 7.5] Runtime failed: "..tostring(result)) end
if not finalPatched then error("[PrimeHub 7.5] Final runtime patch was not applied") end
return result
