-- PrimeHub 8.0 public release assembler
...
local BASE = "https://raw.githubusercontent.com/softiksun5-hue/PrimeHub/main/release/v8.0/"
local FILES = {
...
for i, name in ipairs(FILES) do
    ...
    if not ok or type(body) ~= "string" or #body < 100 then
        error("[PrimeHub 8.0] Could not download release chunk " .. name .. ": " .. tostring(body))
    end
    parts[i] = body
end
...
if #source < 180000 then
    error("[PrimeHub 8.0] Release assembly incomplete: " .. tostring(#source) .. " bytes")
end
...
if not fn then
    error("[PrimeHub 8.0] Release compile failed: " .. tostring(err))
end
return fn()
