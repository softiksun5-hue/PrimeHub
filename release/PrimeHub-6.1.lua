-- PrimeHub 6.1 release bootstrap
-- Fixes 6.0 nested long-string delimiter syntax and keeps real license lifetime UI.
...
if not okBody or type(src)~="string" or #src<100 then
    error("[PrimeHub 6.1] Could not download fresh stable base: "..tostring(src))
end

-- Promote current public/runtime references to 6.1.
src=string.gsub(src,"5%.8","6.1")

-- Inject changes into the stable 5.3 runtime generator before it is compiled.
local anchor='body=string.gsub(body,"5%.3","6.1")\n'
local injection=[======[
-- Keep the public/runtime version separate from the registered license protocol version.
local protocolOld=[==[    src=string.gsub(src,'local PRIMEHUB_VERSION = "5.0"','local PRIMEHUB_VERSION = "6.1"',1)]==]
local protocolNew=[==[    src=string.gsub(src,'local PRIMEHUB_VERSION = "5.0"','local PRIMEHUB_VERSION = "6.1"\nlocal PRIMEHUB_LICENSE_VERSION = "5.0"',1)
...
if not pa then error("[PrimeHub 6.1] License protocol generator target missing") end
...
local uiPatch=[===[
    ...
    local licenseBuild=label(licenseInfo,"Build: PrimeHub 6.1",UDim2.new(1,-20,0,18),UDim2.new(0,12,0,10),9,THEME.muted,true)
...
if not ua then error("[PrimeHub 6.1] License UI generator insertion point missing") end
body=string.sub(body,1,ua-1)..uiPatch..body:sub(ua)
]======]

local a,b=string.find(src,anchor,1,true)
if not a then error("[PrimeHub 6.1] Stable generator anchor missing") end
src=string.sub(src,1,b)..injection..string.sub(src,b+1)

local fn,err=loadstring(src)
if not fn then error("[PrimeHub 6.1] Release compile failed: "..tostring(err)) end
local ok,result=pcall(fn)
if not ok then error("[PrimeHub 6.1] Runtime failed: "..tostring(result)) end
return result
